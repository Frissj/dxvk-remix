...
#include <cstring>
#include <atomic>
#include <functional>  // NV-DXVK: queue lock callbacks, see setQueueLockFunctions
#include <mutex>
#include <span>
#include <vector>
...
public:
  ...
  // NV-DXVK start: the sample owns its queue exclusively, but in Remix the graphics queue is
  // shared with dxvk's submit thread. vkQueueBindSparse and vkQueueWaitIdle are queue
  // operations requiring external synchronization, so Resources installs dxvk's queue lock
  // here and bindLargeBufferChunks holds it across them. Mirrors the submitLockFn /
  // submitUnlockFn pair used by Resources::tempSyncSubmit. Unset means single-owner (the
  // sample's behaviour) and the calls run unguarded.
  void setQueueLockFunctions(std::function<void()> lockFn, std::function<void()> unlockFn)
  {
    m_queueLockFn   = std::move(lockFn);
    m_queueUnlockFn = std::move(unlockFn);
  }
  // NV-DXVK end
...
private:
  ...
  // NV-DXVK: see setQueueLockFunctions
  std::function<void()> m_queueLockFn;
  std::function<void()> m_queueUnlockFn;
