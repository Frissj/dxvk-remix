                                                        ...
                                                        VkFence                  sparseBindingFence) const
{
  ...
  // NV-DXVK start: the queue may be shared with dxvk's submit thread (see
  // setQueueLockFunctions). vkQueueBindSparse requires external synchronization, so hold the
  // lock across it. The fence wait, when a fence was supplied, is deliberately left to the
  // caller so it can happen unlocked - matching Resources::tempSyncSubmit's P4c split.
  if(m_queueLockFn)
  {
    m_queueLockFn();
  }
  ...
  // Without a fence the sample idles the queue here; that is still a queue operation, so it
  // has to stay inside the lock. Callers that care about stalling dxvk pass a fence instead.
  if(result == VK_SUCCESS && !sparseBindingFence)
  {
