...
public:
  ...
  // NV-DXVK P3: appends scene geometries [firstGeometry, firstGeometry +
  // geometryCount) - added to the Scene after init - to the live streaming
  // system in O(new): per-geometry persistent buffers, resident-table entries
  // and (when ray tracing is active) their lowest-detail CLAS/BLAS. Requires
  // StreamingConfig::persistentGeometryCapacity headroom; returns false when
  // the reserved capacity or the geometry table is exhausted (the caller then
  // rebuilds the whole generation). Safe while streaming is live: appended
  // groups occupy reserved IDs and no GPU-visible streaming window moves.
  ...
  // blasReference with its geometry's lowDetailBlasAddress (matches the
  // ScenePreloaded accessor from P2)
...
private:
  ...
  // NV-DXVK P3: initGeometries' per-geometry body, shared with appendGeometries.
  // Creates the persistent buffers, seeds the lowest-detail resident group and
  // fills m_shaderGeometries[geometryIndex]. Returns false when the reserved
  // persistent capacity is exhausted.
  bool initPersistentGeometry(Resources& res, size_t geometryIndex, uint32_t& instancesOffset, Resources::BatchedUploader& uploader);

  // NV-DXVK P3: builds the lowest-detail CLAS + BLAS for persistent groups
  // [firstGeometry, firstGeometry + geometryCount). Used by initClas for the
  // initial set (storing into m_clasLowDetailBuffer/m_clasLowDetailBlasBuffer)
  // and by appendGeometries for appended batches (per-batch buffers below).
  bool buildLowDetailClas(size_t firstGeometry, size_t geometryCount, nvvk::Buffer& clasBuffer, nvvk::Buffer& blasBuffer);
...
private:
  ...
  // NV-DXVK P3: lowest-detail CLAS/BLAS storage of appended geometry batches.
  // Resident buffers are never moved or rebuilt; each append gets its own
  // pair (mirrors ScenePreloaded::m_clasLowDetailBlasBuffers from P2.5).
  std::vector<nvvk::Buffer> m_clasLowDetailAppendBuffers;
  std::vector<nvvk::Buffer> m_clasLowDetailBlasAppendBuffers;
