...
void NVHizVK::TextureInfo::getSize(float size[2]) const
{
  size[0] = float(width);
  size[1] = float(height);
}
...
void NVHizVK::initUpdateViews(Update& update) const
{
  ...
  // NV-DXVK P4: honor the aspect setupUpdateInfos stored instead of assuming a
  // depth-stencil source - Remix feeds an R32_SFLOAT color image (the flipped
  // copy of its primary depth), for which a DEPTH-aspect view would be invalid.
  info.subresourceRange.aspectMask   = update.sourceInfo.aspect;
