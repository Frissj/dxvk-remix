...
struct StreamingConfig
{
  ...
  // NV-DXVK P3: geometry-slot capacity for persistently resident (lowest
  // detail) groups. The resident table reserves group/cluster IDs and
  // active-list slots for this many geometries up front so that new
  // geometries can be appended while streaming is live (the GPU-visible
  // active window base never moves). 0 = exact scene count at init
  // (sample behavior, appends unsupported).
  uint32_t persistentGeometryCapacity = 0;
};
...
public:
  ...
  // come from the reserved prefix, so this remains valid after streaming went
  // live (geometry appends). Returns nullptr when the reserved capacity is
  // exhausted. Without a capacity this matches the sample's addGroup.
  StreamingResident::Group* addGroupPersistent(GeometryGroup geometryGroup, uint32_t clusterCount, uint32_t triangleCount);

  // NV-DXVK P3: uploads the resident group/cluster table entries for
  // persistent groups appended after uploadInitialState. The appended groups
  // occupy reserved IDs untouched by any in-flight streaming task, and no
  // GPU-visible window field changes, so this is safe while streaming runs.
  void uploadAppendedPersistentGroups(Resources::BatchedUploader& uploader, uint32_t firstGroupResidentID, uint32_t groupCount);
...
private:
  ...
  // NV-DXVK P3: persistent-capacity model (0 capacity = sample behavior).
  // Group IDs [0, capacity), cluster IDs [0, capacity) and active-list slots
  // [0, capacity) are reserved for persistent lowest-detail groups; streamed
  // groups occupy slots starting at m_activeStreamedBase, which never moves
  // after uploadInitialState - the key property that makes geometry appends
  // invisible to in-flight streaming tasks.
  uint32_t m_persistentGroupCapacity   = 0;
  uint32_t m_persistentClusterCapacity = 0;
  uint32_t m_persistentGroupsAdded     = 0;
  uint32_t m_persistentClustersAdded   = 0;
  uint32_t m_activeStreamedBase        = 0;
  bool     m_initialStateUploaded      = false;
