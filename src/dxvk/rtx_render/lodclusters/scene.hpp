...
public:
  ...
  // NV-DXVK start: Remix mesh input path (replaces glTF loading for game / replacement geometry)

  // One mesh fed from Remix (game draw-call CPU snapshot, replacement asset, or Remix-API mesh).
  // All pointers reference caller-owned memory that must stay alive for the duration of
  // initFromMeshInputs. Strides are in floats. Null attribute pointers mean "not present".
  struct RemixMeshInput
  {
    std::string name;

    // triangle list, 0-based indices into this mesh's vertices
    std::span<const uint32_t> indices;

    uint32_t vertexCount = 0;

    const float* positions      = nullptr;  // 3 floats each
    uint32_t     positionStride = 3;
    const float* normals        = nullptr;  // 3 floats each
    uint32_t     normalStride   = 3;
    const float* texcoords0     = nullptr;  // 2 floats each
    uint32_t     texcoord0Stride = 2;
    const float* texcoords1     = nullptr;  // 2 floats each
    uint32_t     texcoord1Stride = 2;
    const float* tangents       = nullptr;  // 4 floats each
    uint32_t     tangentStride  = 4;

    // material state baked into cluster state bits (shading itself is Remix's)
    bool  twoSided    = false;
    bool  alphaMasked = false;
    float alphaCutOff = 0.5f;

    // stable identity of the source data, stored in the cache entry for validation
    uint64_t indicesHash  = 0;
    uint64_t verticesHash = 0;
  };

  // Same contract as init(), but geometry comes from Remix mesh inputs instead of a glTF
  // file. filePath is only used to derive the cache file location (never opened itself).
  Result initFromMeshInputs(const std::filesystem::path&    filePath,
  ...
  // Assembles a renderable multi-geometry Scene from per-geometry .nvsngeo cache
  // files, each previously written by a single-geometry initFromMeshInputs run
  // (the provider's background processing). All geometry data stays memory-mapped
  // in the source files for the Scene's lifetime; geometryID equals the index
  // into cacheFilePaths. This realizes the "generation swap" of the Remix
  // integration: a new Scene (and RenderScene) is built whenever newly processed
  // geometries should join rendering.
  Result initFromCachedGeometries(std::span<const std::filesystem::path> cacheFilePaths,
                                  const SceneConfig&                     config,
                                  const SceneLoaderConfig&               loaderConfig);

  // P2.5: extends a Scene built by initFromCachedGeometries with additional
  // per-geometry cache files, in place. Existing geometry views and their
  // geometryIDs are untouched (the mapped data pointers are stable); new
  // geometries get the next indices. Transactional: on any failure the Scene
  // is left exactly as it was. Caller guarantees the files come from the same
  // SceneConfig digest directory as the initial set.
  Result appendCachedGeometries(std::span<const std::filesystem::path> cacheFilePaths);
  // NV-DXVK end
...
private:
  ...
  // NV-DXVK: mappings held by initFromCachedGeometries (one .nvsngeo per
  // geometry); the geometry views alias into these for the Scene's lifetime.
  std::vector<nvutils::FileReadMapping> m_remixCacheMappings;
  ...
  // NV-DXVK start: Remix mesh input path
  Result loadFromMeshInputs(ProcessingInfo& processingInfo, std::span<const RemixMeshInput> inputs);
  void   loadGeometryRemix(ProcessingInfo& processingInfo, uint64_t geometryIndex, const RemixMeshInput& input);
  // NV-DXVK end
