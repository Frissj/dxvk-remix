...
bool SceneStreaming::init(Resources* resources, const Scene* scene, const StreamingConfig& config)
{
  ...
  // NV-DXVK P3: the persistent capacity (>= current geometry count) takes the
  // role of the geometry count in the sizing so appended geometries fit
  m_config.persistentGeometryCapacity =
      std::max(m_config.persistentGeometryCapacity, uint32_t(scene->getActiveGeometryCount()));
  m_config.maxGroups = std::max(m_config.maxGroups, m_config.persistentGeometryCapacity);
  if(m_config.maxClusters == 0)
  {
    m_config.maxClusters = config.maxGroups * scene->m_config.clusterGroupSize;
  }
  m_config.maxClusters =
      std::max(m_config.maxClusters, m_config.persistentGeometryCapacity * scene->m_config.clusterGroupSize);
  ...
  // NV-DXVK P3: geometry-count dependent sizing uses the persistent capacity
  m_updates.init(res, m_config, m_config.persistentGeometryCapacity, groupCountAlignment, clusterCountAlignment);
...
void SceneStreaming::resetCachedBlas(Resources::BatchedUploader& uploader)
{
  ...
  // NV-DXVK P3: ranged - the table buffer is capacity-sized
  uploader.uploadBuffer(m_shaderGeometriesBuffer, 0, sizeof(shaderio::Geometry) * m_shaderGeometries.size(),
...
void SceneStreaming::initGeometries(Resources& res, const Scene* scene)
{
  ...
  {
    // NV-DXVK P3: per-geometry body shared with appendGeometries; cannot fail
    // at init time (the persistent capacity is >= the initial geometry count)
    bool initOk = initPersistentGeometry(res, geometryIndex, instancesOffset, uploader);
    assert(initOk);
    (void)initOk;
  }
  ...
  // NV-DXVK P3: the geometry table is created once at the persistent capacity
  // so appendGeometries can add entries without recreating the buffer (its
  // device address stays valid for in-flight frames); content upload is ranged
  res.createBufferTyped(m_shaderGeometriesBuffer, m_config.persistentGeometryCapacity, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
...
// NV-DXVK P3: verbatim extraction of the sample's initGeometries loop body,
// range-callable so appendGeometries can reuse it for geometries added after
// streaming went live. Only deviation: addGroup -> addGroupPersistent (reserved
// IDs) with failure propagation instead of the init-time-only assert.
bool SceneStreaming::initPersistentGeometry(Resources& res, size_t geometryIndex, uint32_t& instancesOffset, Resources::BatchedUploader& uploader)
{
  const Scene* scene = m_scene;
  {
    ...
    // NV-DXVK P3: reserved-prefix allocation so this also works after
    // streaming went live (appends); nullptr = reserved capacity exhausted
    StreamingResident::Group* rgroup = m_resident.addGroupPersistent(geometryGroup, lastClustersCount, groupInfo.triangleCount);
    if(!rgroup)
    {
      LOGW("SceneStreaming: persistent capacity exhausted at geometry %zu\n", geometryIndex);
      return false;
    }
...
bool SceneStreaming::appendGeometries(size_t firstGeometry, size_t geometryCount)
{
  ...
  // reserved persistent prefix and geometry table are capacity-sized
  if(firstGeometry + geometryCount > m_config.persistentGeometryCapacity)
  {
    LOGI("SceneStreaming::appendGeometries persistent capacity exceeded (%zu > %u)\n", firstGeometry + geometryCount,
         m_config.persistentGeometryCapacity);
    return false;
  }

  // the CLAS input limits and the per-frame new-CLAS scratch were sized with
  // the cluster maxima known at init; appended data exceeding them needs a
  // full rebuild (the caller handles this via the generation fallback)
  if(m_requiresClas
     && (m_scene->m_maxClusterTriangles > m_clasTriangleInput.maxClusterTriangleCount
         || m_scene->m_maxClusterVertices > m_clasTriangleInput.maxClusterVertexCount))
  {
    LOGI("SceneStreaming::appendGeometries cluster maxima exceed init-time CLAS limits - rebuild required\n");
    return false;
  }

  m_shaderGeometries.resize(firstGeometry + geometryCount);
  m_persistentGeometries.resize(firstGeometry + geometryCount);

  Resources::BatchedUploader uploader(res);

  // continue the running instance offsets of the resident set (P2.5 pattern)
  uint32_t instancesOffset = 0;
  if(firstGeometry > 0)
  {
    const shaderio::Geometry& previous = m_shaderGeometries[firstGeometry - 1];
    instancesOffset                    = previous.instancesOffset + previous.instancesCount;
  }

  bool rangeOk = true;
  for(size_t geometryIndex = firstGeometry; geometryIndex < firstGeometry + geometryCount && rangeOk; geometryIndex++)
  {
    rangeOk = initPersistentGeometry(res, geometryIndex, instancesOffset, uploader);
  }

  if(!rangeOk)
  {
    // roll back the partial range: destroy the created buffers and shrink the
    // arrays. Resident-table groups already added by this batch stay allocated
    // but unreferenced - the caller responds to `false` with a full generation
    // rebuild, which re-initializes the whole streaming system anyway.
    // (flush, not abort: abort would leave the uploader's temp command buffer
    // dangling; the flushed uploads only touch buffers destroyed right below)
    uploader.flush();
    for(size_t i = firstGeometry; i < m_persistentGeometries.size(); i++)
    {
      PersistentGeometry& persistentGeometry = m_persistentGeometries[i];
      res.m_allocator.destroyBuffer(persistentGeometry.groupAddresses);
      res.m_allocator.destroyBuffer(persistentGeometry.nodes);
      res.m_allocator.destroyBuffer(persistentGeometry.nodeBboxes);
      res.m_allocator.destroyBuffer(persistentGeometry.lodLevels);
      res.m_allocator.destroyBuffer(persistentGeometry.lowDetailGroupsData);
    }
    m_persistentGeometries.resize(firstGeometry);
    m_shaderGeometries.resize(firstGeometry);
    return false;
  }

  // group-address tables of the new geometries: all invalid except the
  // always-loaded lowest-detail group (ranged resetGeometryGroupAddresses)
  for(size_t geometryIndex = firstGeometry; geometryIndex < firstGeometry + geometryCount; geometryIndex++)
  {
    SceneStreaming::PersistentGeometry& persistentGeometry = m_persistentGeometries[geometryIndex];
    const Scene::GeometryView&          sceneGeometry      = m_scene->getActiveGeometry(geometryIndex);
  ...
  // resident table entries for the appended lowest-detail groups; no
  // GPU-visible streaming window field changes (see StreamingResident)
  m_resident.uploadAppendedPersistentGroups(uploader, uint32_t(firstGeometry), uint32_t(geometryCount));

  // geometry table entries - disjoint range of the capacity-sized buffer,
  // in-flight frames keep reading their own (smaller) geometry count
  uploader.uploadBuffer(m_shaderGeometriesBuffer, sizeof(shaderio::Geometry) * firstGeometry,
                        sizeof(shaderio::Geometry) * geometryCount, m_shaderGeometries.data() + firstGeometry);

  uploader.flush();

  if(m_requiresClas)
  {
    // lowest-detail CLAS + BLAS for the new range into per-batch buffers
    nvvk::Buffer clasBuffer;
    nvvk::Buffer blasBuffer;
    if(!buildLowDetailClas(firstGeometry, geometryCount, clasBuffer, blasBuffer))
    {
      res.m_allocator.destroyBuffer(clasBuffer);
      res.m_allocator.destroyBuffer(blasBuffer);
      return false;
    }
    m_clasLowDetailAppendBuffers.push_back(clasBuffer);
    m_clasLowDetailBlasAppendBuffers.push_back(blasBuffer);
  }

  LOGI("SceneStreaming: appended %zu geometries (%zu total, persistent capacity %u)\n", geometryCount,
       m_persistentGeometries.size(), m_config.persistentGeometryCapacity);
                                                ...
                                                uint32_t             popRequestIndex)
{
  ...
  // vk_lod_clusters c19a250: when the clas storage was grown after this request was
  // recorded, we may actually have more available than its readback reports
...
void SceneStreaming::getStats(StreamingStats& stats) const
{
  ...
  // vk_lod_clusters c19a250: report the live committed size of the sparse CLAS buffer
  if(m_requiresClas)
  {
...
bool SceneStreaming::initClas()
{
  ...
  // vk_lod_clusters c19a250: reservedClasBytes now tracks the committed sparse size and is
  // set after StreamingResident::initClas below
  Resources& res       = *m_resources;
  m_clasOperationsSize = 0;
  m_blasSize           = 0;
  ...
  if(m_config.usePersistentClasAllocator)
  {
    ...
    // vk_lod_clusters c19a250: laid out for the maximum, effective coverage starts at the
    // initial committed size and follows growClas
...

  {
    ...
    // vk_lod_clusters c19a250: the CLAS buffer now exists, report its actual committed size
    m_stats.reservedClasBytes = m_resident.getAllocatedClasBytes();
    if(!m_config.usePersistentClasAllocator)
    {
    ...
    // NV-DXVK P3: the actual build is range-callable so appendGeometries can
    // reuse it verbatim for appended batches
    if(!buildLowDetailClas(0, loGroupsCount, m_clasLowDetailBuffer, m_clasLowDetailBlasBuffer))
    {
      return false;
    }
  }
...
// NV-DXVK P3: verbatim extraction of initClas' lowest-detail CLAS/BLAS build,
// generalized to a geometry range. The initial set builds into the classic
// m_clasLowDetailBuffer/m_clasLowDetailBlasBuffer; appended batches build into
...
bool SceneStreaming::buildLowDetailClas(size_t firstGeometry, size_t geometryCount, nvvk::Buffer& clasBuffer, nvvk::Buffer& blasBuffer)
{
  ...
  {
    // persistent group/cluster IDs are sequential within the reserved prefix
    uint32_t       loGroupsCount           = uint32_t(geometryCount);
    uint32_t       loClustersCount         = 0;
    uint32_t       loMaxGroupClustersCount = 0;
    const uint32_t firstClusterResidentID  = m_resident.getGroup(uint32_t(firstGeometry)).clusterResidentID;
    for(size_t g = 0; g < geometryCount; g++)
    {
      const StreamingResident::Group& group = m_resident.getGroup(uint32_t(firstGeometry + g));
      assert(group.clusterResidentID == firstClusterResidentID + loClustersCount);
      loClustersCount += group.clusterCount;
      loMaxGroupClustersCount = std::max(loMaxGroupClustersCount, uint32_t(group.clusterCount));
    }
    ...
    {
      // NV-DXVK P3: ranged access by resident ID (IDs == geometry indices in
      // the reserved persistent prefix)
      ...
      blasBuildInfos[g].clusterReferences =
          m_shaderData.resident.clasAddresses + sizeof(uint64_t) * (firstClusterResidentID + clusterOffset);


      for(uint32_t c = 0; c < residentGroup.clusterCount; c++)
      {
        const shaderio::Cluster& sceneCluster = sceneGroupView.clusters[c];
        assert((residentGroup.clusterResidentID + c) == firstClusterResidentID + clusterOffset);
    ...
    res.createBuffer(clasBuffer, clasSize,
                     VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR);
    NVVK_DBG_NAME(clasBuffer.buffer);
    // NV-DXVK P3: accumulates across appended batches (zeroed in deinitClas)
    ...
    // NV-DXVK P3: destination offsets account for the range's first cluster ID
    VkBufferCopy region;
    region.srcOffset = 0;
    region.dstOffset = (m_shaderData.resident.clasSizes - residentClasBuffer.address) + sizeof(uint32_t) * firstClusterResidentID;
    region.size      = sizeof(uint32_t) * loClustersCount;

    vkCmdCopyBuffer(cmd, buildSizesHost.buffer, residentClasBuffer.buffer, 1, &region);

    region.srcOffset = 0;
    region.dstOffset = (m_shaderData.resident.clasAddresses - residentClasBuffer.address) + sizeof(uint64_t) * firstClusterResidentID;
    region.size      = sizeof(uint64_t) * loClustersCount;

    vkCmdCopyBuffer(cmd, buildAddressesHost.buffer, residentClasBuffer.buffer, 1, &region);

    // NV-DXVK P3: the device-side allocator holds live allocations once
    // streaming ran - only the initial build may reset it
    if(m_config.usePersistentClasAllocator && firstGeometry == 0)
    {
...

    {
      ...
      res.createBuffer(blasBuffer, blasSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR);
      NVVK_DBG_NAME(blasBuffer.buffer);
      m_blasSize += blasSize;

      cmdInfo.input        = blasInputInfo;
      cmdInfo.input.opMode = VK_CLUSTER_ACCELERATION_STRUCTURE_OP_MODE_IMPLICIT_DESTINATIONS_NV;

      cmdInfo.dstImplicitData = blasBuffer.address;
    ...
    // NV-DXVK P3: ranged upload - the table buffer is capacity-sized and only
    // [firstGeometry, firstGeometry + geometryCount) changed
    res.simpleUploadBuffer(m_shaderGeometriesBuffer, sizeof(shaderio::Geometry) * firstGeometry,
                           sizeof(shaderio::Geometry) * geometryCount, m_shaderGeometries.data() + firstGeometry);
...
void SceneStreaming::deinitClas()
{
  ...
  // NV-DXVK P3: ranged - the table buffer is capacity-sized
  res.simpleUploadBuffer(m_shaderGeometriesBuffer, 0, sizeof(shaderio::Geometry) * m_shaderGeometries.size(),
                         m_shaderGeometries.data());
