...
bool ScenePreloaded::init(Resources* res, const Scene* scene, const Config& config)
{
  ...
  // NV-DXVK P2.5: the geometry table is created once with `geometryCapacity`
  // slots so appendGeometries can add entries without recreating the buffer
  // (its address/descriptor stay valid for in-flight frames). Capacity 0
  // keeps the sample's exact-count sizing.
  const size_t tableEntries = std::max(size_t(m_config.geometryCapacity), scene->getActiveGeometryCount());
  res->createBufferTyped(m_shaderGeometriesBuffer, tableEntries, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
  ...
  return initGeometries(0, scene->getActiveGeometryCount());
}

bool ScenePreloaded::appendGeometries(size_t firstGeometry, size_t geometryCount)
{
  // NV-DXVK P2.5: incremental extension for geometries appended to the Scene
  // after init (Scene::appendCachedGeometries). O(new) work only.
  assert(m_resources != nullptr && m_scene != nullptr && "appendGeometries requires init");
  assert(firstGeometry == m_geometries.size() && "append range must extend the resident set");
  ...
  const size_t tableEntries = m_shaderGeometriesBuffer.bufferSize / sizeof(shaderio::Geometry);
  if(firstGeometry + geometryCount > tableEntries)
  {
    LOGI("ScenePreloaded::appendGeometries geometry table capacity exceeded (%zu > %zu)\n",
         firstGeometry + geometryCount, tableEntries);
    return false;
  }

  // conservative re-estimate over the whole (already appended) scene
  if(!canPreload(m_resources->getDeviceLocalHeapSize(), m_scene))
  {
    LOGW("Likely exceeding device memory limit for preloaded scene\n");
    return false;
  }

  if(!initGeometries(firstGeometry, geometryCount))
  {
    return false;
  }
  ...
  if(m_hasClas)
  {
    return initClasGeometries(firstGeometry, geometryCount);
  }

  return true;
}

bool ScenePreloaded::initGeometries(size_t firstGeometry, size_t geometryCount)
{
  Resources*   res   = m_resources;
  const Scene* scene = m_scene;

  m_shaderGeometries.resize(firstGeometry + geometryCount);
  m_geometries.resize(firstGeometry + geometryCount);

  Resources::BatchedUploader uploader(*res);

  // NV-DXVK P2.5: continue the running instance offsets of the resident set
  ...
  // NV-DXVK P2.5: only this range's table entries are written; resident
  // entries (and the table buffer itself) stay untouched
  uploader.uploadBuffer(m_shaderGeometriesBuffer, firstGeometry * sizeof(shaderio::Geometry),
                        geometryCount * sizeof(shaderio::Geometry), m_shaderGeometries.data() + firstGeometry);
  uploader.flush();
...
void ScenePreloaded::deinit()
{
  ...
  for(nvvk::Buffer& lowDetailBlasBuffer : m_clasLowDetailBlasBuffers)
  {
    m_resources->m_allocator.destroyBuffer(lowDetailBlasBuffer);
  }
  m_clasLowDetailBlasBuffers.clear();
...
bool ScenePreloaded::initClas()
{
  ...
  return initClasGeometries(0, m_geometries.size());
}

bool ScenePreloaded::initClasGeometries(size_t firstGeometry, size_t geometryCount)
{
  Resources* res = m_resources;
  ...
  // NV-DXVK P2.5: the low-detail BLAS build covers this range only
  blasInput.maxClusterCountPerAccelerationStructure = 1;
  blasInput.maxTotalClusterCount                    = uint32_t(geometryCount);
  ...
  blasInputInfo.maxAccelerationStructureCount = uint32_t(geometryCount);
  ...
  res->createBufferTyped(blasBuildInfosHost, geometryCount,
                         VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
                         VMA_MEMORY_USAGE_CPU_ONLY,
                         VMA_ALLOCATION_CREATE_MAPPED_BIT | VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT);
  ...
  res->createBufferTyped(buildSizesHost,
                         std::max(m_scene->m_maxPerGeometryClusters, uint32_t(geometryCount)),
  ...
  for(size_t g = firstGeometry; g < firstGeometry + geometryCount; g++)
  {
...

    {
      ...
      // NV-DXVK P2.5: host array indexed relative to the range
      VkClusterAccelerationStructureBuildClustersBottomLevelInfoNV* blasInfos = blasBuildInfosHost.data();
      // just a single cluster
      blasInfos[g - firstGeometry].clusterReferencesCount  = 1;
      blasInfos[g - firstGeometry].clusterReferencesStride = uint32_t(sizeof(uint64_t));
      blasInfos[g - firstGeometry].clusterReferences =
          preloadGeometry.clusterClasAddresses.addressAt(shaderGeometry.lowDetailClusterID);
    }
  ...
  // NV-DXVK P2.5: one implicit-destination buffer per init/append batch so
  // resident low-detail BLASes are never rebuilt or moved
  {
    nvvk::Buffer clasLowDetailBlasBuffer;
    res->createBuffer(clasLowDetailBlasBuffer, blasSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR);
    NVVK_DBG_NAME(clasLowDetailBlasBuffer.buffer);
    m_clasLowDetailBlasBuffers.push_back(clasLowDetailBlasBuffer);
    m_blasSize += blasSize;
    ...
    cmdInfo.dstImplicitData = clasLowDetailBlasBuffer.address;
    ...
    for(size_t g = firstGeometry; g < firstGeometry + geometryCount; g++)
    {
      ...
      shaderGeometry.lowDetailBlasAddress = blasAddresses[g - firstGeometry];
    }
  }

  // NV-DXVK P2.5: only this range's table entries are (re)written
  m_resources->simpleUploadBuffer(m_shaderGeometriesBuffer, firstGeometry * sizeof(shaderio::Geometry),
                                  geometryCount * sizeof(shaderio::Geometry), m_shaderGeometries.data() + firstGeometry);
...
void ScenePreloaded::deinitClas()
{
  ...
  {
    m_resources->simpleUploadBuffer(m_shaderGeometriesBuffer, 0, m_geometries.size() * sizeof(shaderio::Geometry),
