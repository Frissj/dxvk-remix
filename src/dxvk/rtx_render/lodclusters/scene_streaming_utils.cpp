...
void StreamingResident::init(Resources& res, const StreamingConfig& config, uint32_t groupCountAlignment, uint32_t clusterCountAlignment)
{
  ...
  // IDs [0, capacity) are taken out of the allocators up front so streamed
  // groups can never occupy them; each lowest-detail group holds exactly one
  // cluster (asserted at the seeding sites).
  m_persistentGroupCapacity   = config.persistentGeometryCapacity;
  m_persistentClusterCapacity = config.persistentGeometryCapacity;
  m_persistentGroupsAdded     = 0;
  m_persistentClustersAdded   = 0;
  m_activeStreamedBase        = 0;
  m_initialStateUploaded      = false;
  if(m_persistentGroupCapacity)
  {
    assert(m_persistentGroupCapacity <= config.maxGroups && m_persistentClusterCapacity <= config.maxClusters);
    uint32_t reservedBase = ~0u;
    bool     valid        = m_groupAllocator.createRangeID(reservedBase, m_persistentGroupCapacity);
    assert(valid && reservedBase == 0);
    valid = m_clusterAllocator.createRangeID(reservedBase, m_persistentClusterCapacity);
    assert(valid && reservedBase == 0);
    (void)valid;
    (void)reservedBase;
  }
  ...
  // NV-DXVK P3: streamed active-list slots start at the reserved capacity, so
  // the slot range spans capacity + maxGroups entries
  m_activeGroupIndices.resize(size_t(m_maxGroups) + m_persistentGroupCapacity);
  ...
  // NV-DXVK P3: sized for the reserved persistent prefix + streamed slots
  m_residentActiveOffset   = ranges.append(sizeof(shaderio::uint32_t) * (uint64_t(m_maxGroups) + m_persistentGroupCapacity), 4);
                                                            ...
                                                            uint32_t&                    loMaxGroupClustersCount)
{
  ...
  // vk_lod_clusters c19a250: reserve address space for the full maximum, but only commit the
  // starting chunks. The persistent allocator can grow later; the compaction-based fallback
  // cannot, so it commits everything up front.
...
void StreamingResident::reset(shaderio::StreamingResident& shaderData)
{
  ...
  for(uint32_t streamedIndex = 0; streamedIndex < streamedCount; streamedIndex++)
  {
    Group& group = m_groups[m_activeGroupIndices[m_activeStreamedBase + streamedIndex]];
  ...
  m_shaderData.activeGroups = m_residentBuffer.address + m_residentActiveOffset + sizeof(uint32_t) * m_activeStreamedBase;
...
void StreamingResident::uploadInitialState(Resources::BatchedUploader& uploader, shaderio::StreamingResident& shaderData)
{
  ...
  // NV-DXVK P3: the streamed window base is fixed for the lifetime of this
  // resident table - at the reserved persistent capacity when one is
  // configured (allowing persistent appends), else at the initial persistent
  // count (sample behavior).
  m_activeStreamedBase   = std::max(m_persistentGroupCapacity, m_lowDetailGroupsCount);
  m_initialStateUploaded = true;
...
// NV-DXVK P3: ranged table upload for persistent groups appended after
// uploadInitialState (mirrors the loop above for the new IDs only).
void StreamingResident::uploadAppendedPersistentGroups(Resources::BatchedUploader& uploader,
                                                       uint32_t                    firstGroupResidentID,
                                                       uint32_t                    groupCount)
{
  if(!groupCount)
  {
  ...
  assert(m_initialStateUploaded && m_persistentGroupCapacity);
  assert(firstGroupResidentID + groupCount <= m_lowDetailGroupsCount);

  shaderio::StreamingGroup* shaderGroups = uploader.uploadBuffer(
      m_residentBuffer, m_residentGroupsOffset + sizeof(shaderio::StreamingGroup) * firstGroupResidentID,
      sizeof(shaderio::StreamingGroup) * groupCount, (shaderio::StreamingGroup*)nullptr);

  const uint32_t firstClusterResidentID = m_groups[firstGroupResidentID].clusterResidentID;
  uint32_t       clusterCount           = 0;
  for(uint32_t g = 0; g < groupCount; g++)
  {
    // reserved-prefix IDs are sequential, so the cluster range is contiguous
    assert(m_groups[firstGroupResidentID + g].clusterResidentID == firstClusterResidentID + clusterCount);
    clusterCount += m_groups[firstGroupResidentID + g].clusterCount;
  }

  uint64_t* shaderClusters =
  ...
  for(uint32_t g = 0; g < groupCount; g++)
  {
    ...
    for(uint32_t c = 0; c < group.clusterCount; c++)
    {
      shaderClusters[(group.clusterResidentID - firstClusterResidentID) + c] =
          group.deviceAddress + sizeof(shaderio::Group) + sizeof(shaderio::Cluster) * c;
    }
  }
}
...
StreamingResident::Group* StreamingResident::addGroup(GeometryGroup geometryGroup, uint32_t clusterCount, uint32_t triangleCount)
{
  ...
  // NV-DXVK P3: streamed slots start at m_activeStreamedBase (== the old
  // m_lowDetailGroupsCount when no persistent capacity is reserved, keeping
  // sample behavior bit-identical)
  group.activeIndex       = m_activeStreamedBase + (m_activeGroupsCount - m_lowDetailGroupsCount);
  m_activeGroupsCount++;
...
// NV-DXVK P3: persistent (lowest detail) group addition, valid both while
// seeding the initial state and after streaming went live (geometry appends).
StreamingResident::Group* StreamingResident::addGroupPersistent(GeometryGroup geometryGroup, uint32_t clusterCount, uint32_t triangleCount)
{
  if(!m_persistentGroupCapacity)
  {
    // sample behavior: only valid before uploadInitialState snapshots the
    // persistent set (appends are rejected without a reserved capacity)
    assert(!m_initialStateUploaded);
    return addGroup(geometryGroup, clusterCount, triangleCount);
  }

  if(m_persistentGroupsAdded >= m_persistentGroupCapacity
     || m_persistentClustersAdded + clusterCount > m_persistentClusterCapacity)
  {
    return nullptr;
  }

  const uint32_t groupResidentID   = m_persistentGroupsAdded;
  const uint32_t clusterResidentID = m_persistentClustersAdded;
  m_persistentGroupsAdded++;
  m_persistentClustersAdded += clusterCount;
  ...
  // persistent groups occupy the reserved active-list prefix
  group.activeIndex       = groupResidentID;
  group.geometryGroup     = geometryGroup;
  group.groupResidentID   = groupResidentID;
  group.clusterResidentID = clusterResidentID;
  group.clusterCount      = clusterCount;
  ...
  if(m_initialStateUploaded)
  {
    // append after streaming went live: the group is part of the persistent
    // set immediately; streamed-window counters (active - lowDetail) are
    // unchanged, so no GPU-visible streaming state moves
    m_lowDetailGroupsCount++;
    m_lowDetailClustersCount += clusterCount;
    m_lowDetailTrianglesCount += triangleCount;
    m_lowDetailMaxGroupClusters = std::max(m_lowDetailMaxGroupClusters, clusterCount);
  }
...
void StreamingResident::removeGroup(uint32_t groupResidentID)
{
...

  {
    ...
    // NV-DXVK P3: the streamed region's last slot accounts for the reserved
    // persistent prefix (identical to the sample when no capacity is reserved)
    uint32_t lastSlot = m_activeStreamedBase + (m_activeGroupsCount - m_lowDetailGroupsCount) - 1;

    // classic swapping our position in the active list with last element
    if(activeIndex != lastSlot)
    {
      uint32_t lastResidentID              = m_activeGroupIndices[lastSlot];
...
StreamingAllocator::Layout StreamingAllocator::computeLayout(size_t clasBytes, uint32_t granularityByteSize, uint32_t sectorSizeShift)
{
  ...
  // can only manage memory in multiples of sectorSize, so there might be some initial waste
                              ...
                              shaderio::StreamingAllocator& shaderData)
{
  ...
  // vk_lod_clusters c19a250: management buffer is sized for the reserved maximum, but the
  // shader-visible sectorCount only covers what the sparse CLAS buffer has committed so far
  ...
  // plain buffer offsets, recorded before the device address is folded in below, so
  // growEffective can clear just the newly exposed bitfield tails with vkCmdFillBuffer
  m_usedSectorBitsOffset = VkDeviceSize(m_shaderData.usedSectorBits);
  m_usedBitsOffset       = VkDeviceSize(m_shaderData.usedBits);
